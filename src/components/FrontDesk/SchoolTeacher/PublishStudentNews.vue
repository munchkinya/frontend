    ...
    <div>
        ...
        <el-card>
            ...
            <el-table :data="newsList" :default-sort = "{prop: 'publishtime', order: 'descending'}" >
                ...
                <el-table-column label="新闻标题"  show-overflow-tooltip width="300px">
                    <template slot-scope="scope">
                        <router-link :to="{name: 'frontnewsdetail', params: {id: scope.row.notice_id}}"
                                     style="text-decoration: none;color: #409EFF">{{scope.row.title}}</router-link>
                    </template>
                </el-table-column>
...
<script>
    export default {
        ...
        methods: {
            async getList(){
                const result=await this.$http.get('news/getallstudentNewsbypublisher',
                    {
                        params: {
                            pageNum: this.queryInfo.pagenum,
                            pageSize: this.queryInfo.pagesize,
                            query: window.sessionStorage.getItem('username')
                        }
                    });
            ...
            //跳转添加页面
            toaddnews(){
                this.$router.push({name: "frontaddnews", params: {type: 'student'}})
            },
            //跳转编辑页面
            showEditDialog(id){
                this.$router.push({name: "fronteditnews", params: {id: id}})
            },
            //表格中日期的格式化开始日期
